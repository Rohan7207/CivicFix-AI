...
- Backend validates the AI result before changing database relationships
...
- The Master Issue and individual complaints may have different lifecycle responsibilities; linking a complaint to a Master Issue does not replace the complaint's own status.
...
- admin-only Master Issue management APIs are available for creating, viewing, updating, and reviewing Master Issue records
